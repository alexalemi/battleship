...
///////Output
			...
			DesN=DesN%10;
			SubN=SubN%10;
			CarrierN=CarrierN%10;
			PatrolN=PatrolN%10;

			fprintf(stdout, "%c%i%c %c%i%c %c%i%c %c%i%c %c%i%c", CarrierAnew, CarrierN , CarrierDirectionnew, BattleAnew, BattleN, BattleDirectionnew, DesAnew, DesN, DesDirectionnew, SubAnew, SubN, SubDirectionnew, PatrolAnew, PatrolN, PatrolDirectionnew);
...
*/
			...
			char lastAnew;
			lastAnew = convert(lastA+1);
			int lastNnew = lastN%10;
			fprintf(stdout, "%c%i", lastAnew, lastNnew);
